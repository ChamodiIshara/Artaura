...
{{csrf_field() }}
    ...
    <div class="row">
            ...
            <input type="text" id="firstname" value="{{Auth::user()->name}}" name="firstname" placeholder="user"required>
</div>
            <div class="col-50">
            <label for="lastname"><i class="fa fa-user"></i> Last Name</label>
            <input type="text" id="lastname"value="{{Auth::user()->name}}" name="lastname" placeholder="user2"required>
            </div>
              <div class="col-50">
            <label for="email"><i class="fa fa-envelope"></i> Email</label>
            <input type="text" id="email"value="{{Auth::user()->email}}" name="email" placeholder="user@example.com"required>
</div>
<div class="col-50">
            <label for="address"><i class="fa fa-map-marker"></i> Address</label>
            <input type="text" id="adr"value="{{Auth::user()->address}}" name="address" placeholder="542 W. 15th Street"required>
</div>
<div class="col-50">
            <label for="phone"><i class="fa fa-phone"></i> Phone</label>
            <input type="text" id="phone"value="{{Auth::user()->phone}}" name="phone" placeholder="+94756685178"required>
</div>
<div class="col-50">
            <label for="city"><i class="fa fa-institution"></i> City</label>
            <input type="text" id="city"value="{{Auth::user()->city}}" name="city" placeholder="New York"required>
</div>
              <div class="col-50">
                <label for="state"><i style="font-size:16px" class="fa">&#xf0ac;</i>Country</label>
                <input type="text" id="state"value="{{Auth::user()->state}}" name="state" placeholder="NY"required>
              </div>
              <div class="col-50">
                <label for="zip"><i class="fa fa-file-zip-o"></i>
Zip</label>
                <input type="text" id="zip"value="{{Auth::user()->zip}}" name="zip" placeholder="100"required>
              </div>
...
@foreach($cart as $carts)
    ...
    <td style="padding:5px;color:white;">
        ...
        <td style="padding:5px;color:white;"align="center"><a class="btn btn-danger" href="{{url('delete',$carts->id)}}">Delete</a></td>
    </tr>
    @php $total +=$carts->price*$carts->quantity  @endphp
    @endforeach
  </table>
    <div style="padding:10px;background-color:maroon;color:white;width:351px;">
    <h6> Total price : <span style="padding:100px;"> $ {{$total}}</span></h6>
</div>     
</form>
          </div>
    </div>
